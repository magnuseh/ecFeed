...
public class XomParser {
	...
	public TestCaseNode parseTestCaseElement(Element element, List<AbstractCategoryNode> categories) throws ParserException {
		...
		for(int i = 0; i < parameterElements.size(); i++){
			...
			if(testParameterElement.getLocalName().equals(TEST_PARAMETER_NODE_NAME)){
				...
				testValue = new PartitionNode(EXPECTED_VALUE_PARTITION_NAME, value);
	...
	public ExpectedCategoryNode parseExpectedValueCategoryElement(Element element) throws ParserException {
		validateType(element, EXPECTED_VALUE_CATEGORY_NODE_NAME);

		String name = getElementName(element);
		String type = getAttributeValue(element, TYPE_NAME_ATTRIBUTE);
		String defaultValueString = getAttributeValue(element, DEFAULT_EXPECTED_VALUE_ATTRIBUTE);
		Object defaultValue = parseValue(defaultValueString, type);
		return new ExpectedCategoryNode(name, type, defaultValue);
	}

	public PartitionedCategoryNode parseCategoryElement(Element element) throws ParserException {
	...
	public PartitionNode parsePartitionElement(Element element, String typeSignature) throws ParserException {
		validateType(element, PARTITION_NODE_NAME);

		String name = getElementName(element);
		String valueString = getAttributeValue(element, VALUE_ATTRIBUTE);

...
public class ObsoleteCoverageSetupDialog extends TitleAreaDialog {
	...
	public ObsoleteCoverageSetupDialog(Shell parentShell, MethodNode method, int content, String title, String message,
	...
	private void createTestCaseViewer(Composite parent) {
		...
		fTestCasesViewer.addCheckStateListener(new GraphUpdatingTreeCheckStateListener(fTestCasesViewer, this));

	}

	private class GraphUpdatingTreeCheckStateListener extends TreeCheckStateListener {
		private ObsoleteCoverageSetupDialog fDialog;

		public GraphUpdatingTreeCheckStateListener(CheckboxTreeViewer treeViewer, ObsoleteCoverageSetupDialog dialog) {
			super(treeViewer);
			this.fDialog = dialog;
		}
	...
	protected List<List<PartitionNode>> getSelectedTestCases() {
		...
		for (Object obj : fTestCasesViewer.getCheckedElements()) {
			if (obj.getClass().isInstance(tcnode)) {
				...
				String testSuiteName = (String) obj;
				ArrayList<TestCaseNode> suiteCases = new ArrayList<>();
				suiteCases.addAll(fMethod.getTestCases(testSuiteName));
				if (suiteCases.size() >= 500) {
					for (TestCaseNode tcsuite : suiteCases) {
						List<PartitionNode> partitions = new ArrayList<>();
						for (PartitionNode pnode : tcsuite.getTestData()) {
							partitions.add(pnode);
						}
						cases.add(partitions);
					}
	...
	private void updateCoverage() {
		for (int n = 1; n <= N; n++) {
			int covered = fCoverageUtils.calculateCoveredTuples(getSelectedTestCases(), n);
			if (fTotalWork[n - 1] == 0)
				fResults[n - 1] = 0;
			else
